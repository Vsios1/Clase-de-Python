...
class MemoryManager:
    def __init__(self, total_ram_bytes=1024, page_size_bytes=64, cache_capacity_pages=32):
        ...
        # pid -> pages allocated in RAM
        self.page_table = {}
        # pid -> pages swapped out
        self.swap_table = {}
        # Simple LRU cache for recently used pages (pid, page_index)
    ...
    def allocate(self, pid, bytes_requested):
        ...
        # Try to free pages from swap if available space is insufficient
        while self.free_pages < pages_needed:
            # Swap out least-recently-used page from any process
            if not self.cache:
                # If no cache entry, swap from largest process in RAM
                ...
                victim_key, _ = self.cache.popitem(last=False)  # least-recently-used
    ...
    def access(self, pid, page_index):
        # Access a page, possibly swap in
        if self.page_table.get(pid, 0) == 0 and self.swap_table.get(pid, 0) > 0:
            # Lazy swap-in of one page
            if self.free_pages == 0:
                # Evict one page via LRU
                if self.cache:
